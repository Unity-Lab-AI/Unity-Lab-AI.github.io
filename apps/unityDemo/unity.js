...
const DEFAULT_INSTRUCTION = "All code must be wrapped in [CODE]...[/CODE] tags." +
  "\n\nWhen generating images, selfies, pics, photographs, etc use this exact format: *generates image of [detailed description]*\n" +
  "Example: *generates image of a red apple on a wooden table with dramatic lighting*\n\n" +
...
// Evil persona now uses PollinationsAPI.EVIL_SYSTEM_PROMPT from pollylib.js
...
async function buildMessages(modelName, userMessage) {
  ...
  const imageEnabledMessage = userMessage;
  messages.push({
    role: "user",
...
async function unityDemoGetCaption(imagePrompt, originalUserMsg, canonical) {
...
async function unityDemoSendSelfImageRequest(message) {
  ...
  const caption = await unityDemoGetCaption(imagePromptText, message, canonical);
...
async function sendMessage(message) {
  ...
  try {
    ...
    if (selectedModel === "unity" || selectedModel === "evil") {
      apiModel = "mistral";
      // Prepend appropriate system prompt to messages
      const systemPrompt = selectedModel === "unity"
        ? (PollinationsAPI.UNITY_SYSTEM_PROMPT || UNITY_PERSONA)
        : (PollinationsAPI.EVIL_SYSTEM_PROMPT || EVIL_PERSONA);
      messages.unshift({ role: "system", content: systemPrompt + "\n\n" + DEFAULT_INSTRUCTION });
    } else {
      // For base models, still add image generation instructions
      messages.unshift({ role: "system", content: DEFAULT_INSTRUCTION });
    }
    ...
    // Add any pending images via DOM (bypasses sanitization)
    // Images are appended AFTER the text content
    // Deduplicate images - only show one image per unique prompt
    console.log('Pending images before render:', window.pendingChatImages);
    if (window.pendingChatImages && window.pendingChatImages.length > 0) {
      // Dedupe by checking if URLs contain same encoded prompt
      const seenPrompts = new Set();
      const uniqueImages = window.pendingChatImages.filter(imgData => {
        // Extract the prompt part from URL for comparison (handles both /prompt/ and /image/ paths)
        const promptMatch = imgData.url.match(/\/(?:prompt|image)\/([^?]+)/);
        if (!promptMatch) return true; // Keep if can't parse

        // Decode and normalize the prompt for comparison
        let promptKey;
        try {
          promptKey = decodeURIComponent(promptMatch[1]).toLowerCase().substring(0, 50);
        } catch (e) {
          promptKey = promptMatch[1].toLowerCase().substring(0, 50);
        }

        if (seenPrompts.has(promptKey)) {
          console.log('Skipping duplicate image:', promptKey);
          return false; // Skip duplicate
        }
        seenPrompts.add(promptKey);
        return true;
      });
      console.log('Unique images to render:', uniqueImages.length);
      ...
      uniqueImages.forEach(imgData => {
        ...
        console.log('Setting image src to:', imgData.url);

        // Add error handler
        img.onerror = () => {
          console.error('Image failed to load:', imgData.url);
          img.alt = 'Image failed to load: ' + imgData.alt;
        };

        img.onload = () => {
          console.log('Image loaded successfully:', imgId);
        };
        ...
        btnDiv.className = 'image-action-buttons';
        btnDiv.innerHTML = `
        ...
        aiContent.appendChild(imgContainer); // Append after text
      });
      window.pendingChatImages = [];
    }
    ...
    replayButton.onclick = () => speak(stripImagePatterns(accumulatedResponse));
    ...
    if (voiceEnabled) {
      speak(stripImagePatterns(accumulatedResponse));
    }
  ...
  try {
    ...
    models.forEach(model => {
      ...
      if (modelName && modelName !== 'unity' && modelName !== 'evil') {
        ...
        option.textContent = modelName; // Just show name
        option.title = modelDesc || modelName; // Description as tooltip
...
function stripImagePatterns(text) {
  return text
...
function processMessage(text) {
  ...
  // Store pending images to add via DOM after sanitization
  if (!window.pendingChatImages) window.pendingChatImages = [];
  window.pendingChatImages = [];

  // Detect *generates image of X* patterns and extract to pendingChatImages
  // Also catch variations like *shows X*, *here's an image of X*, etc.
  const imagePatterns = [
    /\*(?:generates?|shows?|creates?|displays?|produces?|here'?s?)\s+(?:an?\s+)?(?:image|pic|photo|picture|selfie)(?:\s+of)?\s*:?\s*([^*]+)\*/gi,
    /\[(?:generates?|shows?|creates?|displays?|produces?|here'?s?)\s+(?:an?\s+)?(?:image|pic|photo|picture|selfie)(?:\s+of)?\s*:?\s*([^\]]+)\]/gi
  ];
  ...
  for (const pattern of imagePatterns) {
    text = text.replace(pattern, (match, imageDescription) => {
      if (imageDescription && imageDescription.trim().length > 2) {
        ...
        console.log('Image pattern detected:', cleanDesc);
        console.log('Generated URL:', imageUrl);
        window.pendingChatImages.push({ url: imageUrl, alt: cleanDesc });
      }
      return ''; // Remove the pattern from text
    });
  }

  // Detect markdown image syntax ![](url) and extract to pendingChatImages
  text = text.replace(/!\[([^\]]*)\]\((https?:\/\/[^)]+)\)/gi, (match, alt, url) => {
    window.pendingChatImages.push({ url: url, alt: alt || 'Generated Image' });
    return ''; // Remove from text
  });

  // Detect ALL Pollinations image URLs (anywhere in text) and extract to pendingChatImages
  text = text.replace(/https:\/\/(?:image\.pollinations\.ai\/prompt|gen\.pollinations\.ai\/image)\/[^\s<>")\]]+/gi, (url) => {
  ...
  // Remove "Generated Image" text that often accompanies URLs
  text = text.replace(/Generated\s*Image/gi, '');

  // Remove leftover emoji buttons text
  text = text.replace(/[📋💾🔄]/g, '');

  // Clean up extra whitespace and empty lines
  text = text.replace(/\n\s*\n\s*\n/g, '\n\n').replace(/^\s+|\s+$/gm, '').trim();

  // If text is empty after removing image patterns, return empty string
  if (!text.trim()) {
    return '';
  }
  ...
  for (let segment of segments) {
    ...
    if (trimmedSegment.match(/^\[CODE\]|^```/i)) {
      ...
      if (
        ...
        // Code block contains just an image URL - add to pending images
        window.pendingChatImages.push({ url: codeContent, alt: 'Generated Image' });
      } else if (codeContent) {
      ...
      // Regular text segment - just wrap in paragraph tags
      if (trimmedSegment) {
        processedText += `<p>${trimmedSegment}</p>`;
      }
    }
  }
