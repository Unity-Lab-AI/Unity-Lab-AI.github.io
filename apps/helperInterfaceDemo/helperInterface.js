...
function helperBuildSelfPrompt(canonical, userText, subject) {
...
async function helperGetCaption(imagePrompt, canonical) {
...
async function helperSendSelfImageRequest(message) {
  ...
  userContent.innerHTML = processMessage(message);
  userDiv.appendChild(userAvatar);
  userDiv.appendChild(userContent);
  chatBox.appendChild(userDiv);
  chatBox.scrollTop = chatBox.scrollHeight;

  const subject = helperExtractImagePrompt(message);
  const canonical = PollinationsAPI.UNITY_SYSTEM_PROMPT;
  const imagePromptText = helperBuildSelfPrompt(canonical, message, subject);
  ...
  const caption = await helperGetCaption(imagePromptText, canonical);
...
async function sendMessage(message) {
  ...
  // refuse on explicit content.
  if (helperDetectImageIntent(message) && helperDetectSelfRef(message)) {
    return await helperSendSelfImageRequest(message);
  }
