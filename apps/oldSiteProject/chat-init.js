...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const appendMessage = ({ role, content, index, imageUrls = [] }) => {
        ...
        if (role === "ai") {
            // Process content: handle CODE blocks, IMAGE tags, and regular text
            let processedContent = content;
            const elements = [];

            // Extract IMAGE tags and generate actual image URLs
            const imageTagRegex = /\[IMAGE\]([\s\S]*?)\[\/IMAGE\]/gi;
            let imageMatch;
            const imagePrompts = [];
            while ((imageMatch = imageTagRegex.exec(content)) !== null) {
                imagePrompts.push({
            ...
            // Remove IMAGE tags from content for text processing
            let textContent = content.replace(imageTagRegex, '').trim();
            ...
            while ((match = codeBlockRegex.exec(textContent)) !== null) {
                ...
                if (matchStart > lastIndex) {
                    const textPart = textContent.substring(lastIndex, matchStart);
                    if (textPart.trim()) {
            ...
            if (imagePrompts.length > 0) {
                ...
                imagePrompts.forEach(imgData => {
                    ...
                    const imageContainer = createImageElement(imageUrl, index);
                    bubbleContent.appendChild(imageContainer);
                });
            }

            // Also handle legacy direct image URLs if present
    ...
    const createImageElement = (url, msgIndex) => {
        ...
        Object.assign(loadingDiv.style, { width: "512px", height: "512px", flexDirection: "column" });
        ...
        let retryCount = 0;
        const maxRetries = 3;
        const retryDelays = [3000, 6000, 10000]; // Increasing delays for rate limits
        ...
        img.onerror = () => {
            if (retryCount < maxRetries) {
                ...
                loadingText.textContent = `Rate limited, retrying in ${delay/1000}s... (${retryCount + 1}/${maxRetries})`;
                retryCount++;
                setTimeout(() => {
                    // Add cache-busting param to force new request
                    const retryUrl = url + `&_retry=${Date.now()}`;
                    loadingText.textContent = "Generating image...";
                    loadImage(retryUrl);
                }, delay);
            } else {
                ...
                errorText.textContent = "⚠️ Image generation failed (rate limited)";
                errorText.style.marginBottom = "10px";
                loadingDiv.appendChild(errorText);
                ...
                retryBtn.addEventListener("click", () => {
                    ...
                    newLoadingText.textContent = "Generating image...";
                    newLoadingText.style.marginTop = "10px";
                    loadingDiv.appendChild(newLoadingText);
                    loadImage(url + `&_retry=${Date.now()}`);
                });
    ...
    const renderStoredMessages = messages => {
        ...
        messages.forEach((msg, idx) => {
            ...
            // IMAGE tags are handled by appendMessage - no URL matching needed
            appendMessage({
                role: msg.role,
                content: msg.content,
                index: idx
    ...
    // Surgically remove DOM message nodes whose dataset.index > keepIndex.
    // Leaves prior message bubbles (and their already-loaded <img> elements) untouched
    // so editing a past message does NOT trigger fresh GETs on every prior image URL.
    const removeMessagesAfter = keepIndex => {
        const toRemove = [];
    ...
    // Replace a single bubble's content node in place — used for AI-message edit so
    // we don't blow away the rest of the chat just to update one reply.
    const replaceBubbleAt = (msgIndex, role, content) => {
    ...
    const editMessage = msgIndex => {
        ...
        if (oldMessage.role === "user") {
            ...
            Storage.updateSessionMessages(currentSession.id, currentSession.messages);
            // SURGICAL TRUNCATION — keep messages 0..msgIndex untouched in DOM (their loaded
            // images stay loaded), drop everything after, then update the edited bubble's text.
    ...
    const setupVoiceInputButton = () => {
        ...
        if (!window._chatInternals.voiceInputBtn && inputButtonsContainer) {
            ...
            // Start with muted icon since mic starts muted
    ...
    // Mic starts muted - don't auto-start speech recognition on page load
    // User must click the mic button to start listening
    ...
    const startVoiceChatSlideshow = () => {
        ...
        const updateImage = () => {
            ...
            const imageBase = window.CLASSIC_IMAGE_BASE || 'https://websiteunityailab.gfourteen7525.workers.dev/image';
            voiceChatImage.src = `${imageBase}/${encodeURIComponent(imagePrompt)}?width=512&height=512&seed=${seed}&nolog=true`;
        };
        updateImage();
