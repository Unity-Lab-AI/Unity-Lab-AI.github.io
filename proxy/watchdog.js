...
const CLAUDE_BIN = process.env.UNITY_CLAUDE_BIN
...
async function main() {
  ...
  function spawnClaude() {
    ...
    log(`spawning ${CLAUDE_BIN} (restart count: ${restartCount})`);
    child = spawn(CLAUDE_BIN, ['--dangerously-skip-permissions'], {
    ...
    child.on('error', (err) => log('CLI spawn error:', err.message));
  }
  ...
  function writeToCLI(text) {
    ...
    try {
      ...
      log('CLI stdin write failed:', e.message);
      return false;
    }
  }
  ...
  // === User stdin → CLI stdin passthrough ===
  if (ARGS.spawn) {
    process.stdin.on('data', (chunk) => {
      if (child?.stdin && !child.stdin.destroyed) {
        try { child.stdin.write(chunk); }
