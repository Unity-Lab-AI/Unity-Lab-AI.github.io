...
> **📦 Downloads section (May 2026):** Site has a `/downloads/` index at `downloads/index.html` (legacy Bootstrap chrome) hosting four free projects with deep per-project pages and direct `https://github.com/Unity-Lab-AI/<repo>` links on every card and deep-page CTA: **Moana Miner** (RavenCoin KawPow GPU miner — `downloads/moana/` + `downloads/files/Moana v1.0.zip` → `Unity-Lab-AI/Moana`), **Local Unity AI** v2.5 (offline LLM + SDXL — `downloads/Local Unity/` + `Local Unity v2.5.zip` → `Unity-Lab-AI/Local-Unity`), **Claude Code Workflow** (.claude/ template — `downloads/claude/` + `.claude.zip` → `Unity-Lab-AI/UAL-ClaudeWorkflow`), and **The Weird Project** v1.0 (SEX SLAVE DUNGEON — persistent city-builder dungeon harem game on local Ollama + Kokoro TTS + optional Pollinations — `downloads/weird/` + `weird.zip` → `Unity-Lab-AI/Weird`). Header link **Downloads** restored to `redesign/v-d-chrome.jsx` `NAV_LINKS` (sits between Projects and Contact) so the link propagates across every redesigned page (`index`, `ai`, `about`, `apps`, `services`, `projects`, `contact`, `codex`, `terms`, `privacy`). `/downloads/weird/` + `/downloads/` lastmod-bumped in `sitemap.xml` and `scripts/generate-sitemap.js` `PAGE_CONFIG`. **Universal 18+ age gate + ToS/Privacy acceptance modal** (the same `apps/age-verification.js` shared with `/ai/demo/` and `/apps/`) is now wired into all 5 downloads pages (`downloads/index.html`, `downloads/moana/`, `downloads/Local Unity/`, `downloads/claude/`, `downloads/weird/`) at `?v=20260513a` — universal localStorage flags (`button18`, `birthdate`, `husdh-f978dyh-sdf`, `legalAccepted`, `legalAcceptedVersion=v1.0`, `legalAcceptedDate`) mean passing once on demo, apps, or downloads unlocks every other gated path.
