...
> **⚖️ Legal pages (May 2026):** Site ships a full Terms of Service at `/terms.html` (Codex 03 — `redesign/terms-v1.jsx`) and Privacy Policy at `/privacy.html` (Codex 04 — `redesign/privacy-v1.jsx`), built on the gothic V-D chrome with shared `redesign/legal-v1.css`. ToS defers all responsibility to the end user, restricts to adults at the age of consent in their jurisdiction, defers minor-supervision to parents/guardians, disclaims Unity AI Lab and its parties of responsibility for any AI-generated text/audio/image content (deferred to Pollinations and its upstream providers), explicitly states UnityAILab is **NOT a generative AI provider** (just a proof-of-concept red-team showcase), and applies Delaware governing law + AAA arbitration + class-action waiver as the neutral default for a non-monetized multi-state hobby project. Privacy Policy explains no AI conversations or generations are stored on our side, the Pollinations data flow, localStorage usage, the third-party visitor counter, and standard GDPR/CCPA rights gestures. Both pages include a plain-English under-18 callout addressed directly to minor readers. Legal contact: `contact@unityailab.com`. Footer links to both propagate via `redesign/v-d-sections.jsx` `GothicFooter`.

> **📦 Downloads section (May 2026):** Site has a `/downloads/` index at `downloads/index.html` (legacy Bootstrap chrome) hosting three free projects with deep per-project pages and direct `https://github.com/Unity-Lab-AI/<repo>` links on every card and deep-page CTA: **Moana Miner** (RavenCoin KawPow GPU miner — `downloads/moana/` + `downloads/files/Moana v1.0.zip` → `Unity-Lab-AI/Moana`), **Local Unity AI** v2.5 (offline LLM + SDXL — `downloads/Local Unity/` + `Local Unity v2.5.zip` → `Unity-Lab-AI/Local-Unity`), and **The Weird Project** v1.0 (SEX SLAVE DUNGEON — persistent city-builder dungeon harem game on local Ollama + Kokoro TTS + optional Pollinations — `downloads/weird/` + `weird.zip` → `Unity-Lab-AI/Weird`). The fourth-project "Claude Code Workflow" / `.claude.zip` slot was nuked on 2026-05-13 — it pointed at a private proprietary `Unity-Lab-AI/UAL-ClaudeWorkflow` repo and the download itself didn't work. Header link **Downloads** restored to `redesign/v-d-chrome.jsx` `NAV_LINKS` (sits between Projects and Contact) so the link propagates across every redesigned page (`index`, `ai`, `about`, `apps`, `services`, `projects`, `contact`, `codex`, `terms`, `privacy`). `/downloads/weird/` + `/downloads/` lastmod-bumped in `sitemap.xml` and `scripts/generate-sitemap.js` `PAGE_CONFIG`. **Universal 18+ age gate + ToS/Privacy acceptance modal** (the same `apps/age-verification.js` shared with `/ai/demo/` and `/apps/`) is wired into the surviving 4 downloads pages (`downloads/index.html`, `downloads/moana/`, `downloads/Local Unity/`, `downloads/weird/`) at `?v=20260513a` — universal localStorage flags (`button18`, `birthdate`, `husdh-f978dyh-sdf`, `legalAccepted`, `legalAcceptedVersion=v1.0`, `legalAcceptedDate`) mean passing once on demo, apps, or downloads unlocks every other gated path.
